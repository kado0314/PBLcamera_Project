...
# APIキーが設定されているか起動時に確認 (gemini_analyzer.py内でもチェックされるが念のため)
if not os.environ.get("GEMINI_API_KEY"):
    print("警告: 環境変数 'GEMINI_API_KEY' が設定されていません。")
    # 実際の運用ではここで起動を停止させることもできます
    # raise ValueError("GEMINI_API_KEY must be set.")

@app.route('/')
def index():
    """ 3.1: index.html (アップロードフォーム) を表示 """
    return render_template('index.html')

@app.route('/upload', methods=)
def upload():
    """ 3.2: 画像アップロードとAI分析の処理 """
    ...
    try:
        # 3.2 (Insight 3.2a): ファイルをインメモリで読み込む (ディスクI/Oなし)
        ...
        # 3.3 (Insight 3.3a): 汎用エラーハンドリング
        # APIエラー、画像デコードエラー、JSONパースエラーなど全てを捕捉
        return render_template('error.html', error=str(e))

if __name__ == '__main__':
    # このブロックは 'python app.py' でローカルテストする時のみ実行される
    # Gunicorn  はこのブロックを実行しない
